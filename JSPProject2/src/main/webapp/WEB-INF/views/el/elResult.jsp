...
<body>
    ...
    <p>
        address(JSP표현식) : <%= request.getAttribute("address")%>  <br>  <%-- 서울시 중구 남대문로 120 --%>
        address(EL) : ${address}  <br>  <%-- 서울시 중구 남대문로 120 --%>
 
        score : ${score} <br> <%-- 100 --%>
        strList : ${strList} <br> <%-- [가가가,나나나,다다다] --%>
        book : ${book} <br>  <%-- edu.kh.jsp.model.dto.Book@7320fd73 --%>
...
<%-- 
 ...
 --%>
        ...
        <%-- EL은 import 필요없다 --%>
        book의 title필드(EL) : ${book.title}  <br> <%-- 어린왕자 --%>
        book의 writer(EL) : ${book.writer}  <br> <%-- 생택쥐베리 --%>
        book의 price(EL) : ${book.price}  <br> <%-- 4000 --%>
    </p>
